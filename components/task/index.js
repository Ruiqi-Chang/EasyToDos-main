...
const Task = (props) => {
  ...
  const completeTask = () => {
    firestore()
      ...
      .update({
        ...
        completedate: props.item.completed ? null :  firestore.FieldValue.serverTimestamp() ,
      });
  };
  const deleteTask = () => {
    firestore()
  ...
  return (
    <TouchableOpacity style={styles.row} onPress={() => completeTask()}>
      ...
      {!editMode ? (
        <>{props.item.completedate ?(<Text style={props.item.completed ? styles.completedTaskText : styles.taskText}>
            {props.item.task+ " was completed at "+ props.item.completedate.toDate().toString()}
          </Text>): (<Text style={props.item.completed ? styles.completedTaskText : styles.taskText}>
            {props.item.task}
          </Text>)}
          {props.item.completed ?(
          <Icon name="edit" size={0} color="#8B8B8B" />):(<TouchableOpacity style={styles.smallIcon} onPress={() => doEdit()}>
